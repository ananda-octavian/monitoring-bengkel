    ...
    public function resumeview(Request $request)
{
    ...
    public function stopWip(Request $request)
{
    $nospp = $request->nospp;

    // Cari WIP aktif (yang belum di-stop)
    $wip = Wip::where('nospp', $nospp)
    ...
    // Redirect ke halaman manajemenwip dengan data parameter + pesan sukses
    return redirect()->route('manajemenwip', [
        'nospp' => $nospp,
    ...
    ])->with('success', 'WIP berhasil dihentikan.');
}
