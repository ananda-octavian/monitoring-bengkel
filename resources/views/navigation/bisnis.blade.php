    ...
    <div class="container-fluid">
        <div class="row header-container mb-4">
            <div class="col d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
                <h1 class="mb-3 mb-md-0">PADMA BUSINESS</h1>
            <form action="{{ route('logout') }}" method="GET">
                <button type="submit" class="btn btn-secondary back-button">Logout</button>
            </form>
            </div>
        </div>
        @foreach($workshops as $workshop)
            <form action="{{ route('manufaktur') }}" method="GET">
                <input type="hidden" name="bisnis" value="{{ $workshop->bisnis }}">
                <button type="submit" class="branch-button">{{ $workshop->bisnis }}</button>
            </form>
            @endforeach
            <a href="{{ route('manajemen') }}" class="branch-button">BUSINESS REGISTRATION</a>
    </div>
</body>
</html>
@endsection