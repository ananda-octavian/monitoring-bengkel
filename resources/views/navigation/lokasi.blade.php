    ...
    <div class="container-fluid">
        <div class="row header-container mb-4">
            <div class="col d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
                <h1 class="mb-3 mb-md-0">{{ $bisnis }}->{{ $manufaktur }}->{{ $dealer }}->{{ $cabang }}</h1>
        ...
        @foreach($workshops as $workshop)
            <form action="{{ route('home') }}" method="GET">
                ...
                <input type="hidden" name="lokasi" value="{{ $workshop->lokasi }}">
                <button type="submit" class="branch-button">{{ $workshop->lokasi }}</button>
            </form>
            @endforeach
    </div>
</body>
