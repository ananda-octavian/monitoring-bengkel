                        ...
                        <tbody>
                            ...
                            <tr class="{{ $isLate ? 'late-row' : '' }} {{ $isJobDispatchLate ? 'job-dispatch-late' : '' }}">
                                ...
                                <td style="position: sticky; right: 0; background: #fff; z-index: 1;">
                                    <a href="#" class="btn open-modal" data-toggle="modal" data-target="#Modal1{{ $spp->nospp }}">></a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    ...
                    <div class="col d-flex justify-content-between align-items-center">
                        ...
                        @if(auth()->user()->level !== 'manajemen')
                            @if(!$latestWip || $latestWip->stopped_at !== null)
                                ...
                                <a href="{{ route('wipscanin', ['nospp' => $spp->nospp, 'bisnis' => $bisnis, 'manufaktur' => $manufaktur, 'dealer' => $dealer, 'cabang' => $cabang, 'lokasi' => $lokasi]) }}" class="btn btn-secondary">START</a>
                            @else
                                <!-- Show "STOP" button if there's an active WIP without a stopped_at timestamp -->
                                <a href="{{ route('wipscanout', ['nospp' => $spp->nospp, 'bisnis' => $bisnis, 'manufaktur' => $manufaktur, 'dealer' => $dealer, 'cabang' => $cabang, 'lokasi' => $lokasi]) }}" class="btn btn-secondary">STOP</a>
                            @endif
                        @endif
                    </div>
